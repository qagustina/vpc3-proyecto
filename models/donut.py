...
from transformers import (
    ...
    Seq2SeqTrainer,
    default_data_collator, DonutProcessor
)
import evaluate

from models import BaseOCRModel
from models.base import evaluate_predictions, BaseOCRDataset


class DonutOCRDataset(BaseOCRDataset):
...
class DonutModel(BaseOCRModel):
    """Donut model implementation using HuggingFace Transformers."""
    ...
    def load_model(self, model_path: Optional[str] = None):
        ...
        if model_path and os.path.exists(model_path):
            self.processor = DonutProcessor.from_pretrained(model_path, do_resize=True, size=(768, 768),use_fast=True)
    ...
    def train(self,
        ...
        # Create datasets
        train_dataset = DonutOCRDataset(train_dataset_path, self.processor)
        eval_dataset = DonutOCRDataset(eval_dataset_path, self.processor)
    ...
    def _compute_metrics(self, pred):
        ...
        # Apply stripping for cleanliness, as done in your manual_evaluate
        pred_str = [s.strip() for s in pred_str]
        label_str = [s.strip() for s in label_str]

        # Calculate CER directly using jiwer.cer
        # jiwer.cer expects lists of strings for both predictions and references.
        calculated_cer = cer(label_str, pred_str)  # Pass references first, then predictions

        return {"cer": calculated_cer}
        ...
        with torch.no_grad():
            ...
            for step in range(128):  # Max length
                ...
                if decoder_outputs.cross_attentions:
                    ...
                    attention_2d = self._reshape_attention_to_spatial(token_attention,
